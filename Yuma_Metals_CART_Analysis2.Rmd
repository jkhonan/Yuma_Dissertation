...
Metals_CART <- Metals_CART %>%
  rename(
    ...
    "Have Health Coverage" = Health_care,
...
png("CART_Cu_plot.png2", width=2000, height=1100, res=300)
...
37/40 #Number correctly predicted = 92.5%
```
...
png("CART_Hg_plot2.png", width=5000, height=1500, res=300)
...
#Testing how well tree predicts the test data
pred.Hg_pruned <- predict(model.Hg.pruned, test.data, type="class")
table(test.data["High Hg"])
table(pred.Hg_pruned)
sum(!is.na(pred.Hg_pruned))
39/41 #Number correctly predicted = 95.1%
```
...
png("CART_Mn_plot2.png", width=2000, height=1100, res=300)
...
38/41 #Number correctly predicted = 92.7%
```
...
png("CART_Pb_plot2.png", width=2000, height=1100, res=300)
...
#Testing how well tree predicts the test data
pred.Pb_pruned <- predict(model.Pb.pruned, test.data, type="class")
table(test.data["High Pb"])
table(pred.Pb_pruned)
sum(!is.na(pred.Pb_pruned))
39/40 #Number correctly predicted = 97.5%
```
...
png("CART_U_plot2.png", width=2000, height=1100, res=300)
