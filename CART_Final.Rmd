...
#Metals$Result_C_ng_mL <- as.numeric(Metals$Result_C_mg_dL)*10000
#Metals$C_Corrected_P <- as.numeric(Metals$Result_P_ng_mL)/Metals$Result_C_ng_mL

#Simplifying income for analysis
#at or below poverty
#above pov but below median? $44,116 in 2018
#at or above 2018 median

Metals$Simplified_Income <- ifelse(Metals$Below_Pov==1, 1, 
                                     ifelse(Metals$Below_Pov==0 & Metals$Income<7, 2, 
                                            ifelse(Metals$Income>6 & Metals$Income<11, 3, NA)))
...
#Removing refused or unknown answers
unique(Metals_CART$Race) #Almost entirely white, other than one native hawaiian/pacific islander, other, don't know, and refused. Removing from analysis.
...
###Cadmium (Removed due to highly censored data)
```{r cadmium}
...
#  dplyr::select(-`Log10 Copper`, -`Log10 Manganese`, -`Log10 Mercury`, -`Log10 Lead`, -`Log10 Uranium`)
...
#rpart.plot(model.Cd, fallen.leaves=FALSE, tweak=1.1, varlen=35, faclen=4, uniform=TRUE)
#dev.off()

#png("CART_Cd_plot_pruned.png", width=2300, height=800, res=300)
#model.Cd.pruned <- prune(model.Cd, cp = 0.02)
#par(xpd = NA) # otherwise on some devices the text is clipped
#rpart.plot(model.Cd.pruned, fallen.leaves=FALSE, tweak=1.1, varlen=35, faclen=4, uniform=TRUE)
#dev.off()
...
Metals_CART_Cu <- Metals_CART %>% 
  dplyr::select(-`Log10 Cadmium`, -`Log10 Manganese`, -`Log10 Mercury`, -`Log10 Lead`, -`Log10 Uranium`)
sample(1:100000000,1)
set.seed(93303313)
training.samples <- createDataPartition(Metals_CART_Cu$`Log10 Copper`, times = 1, p = 0.8, list = F)
train.data  <- Metals_CART_Cu[training.samples, ]
test.data <- Metals_CART_Cu[-training.samples, ]

train.data <- as.data.frame(train.data)

png("CART_Cu_plot.png", width=2500, height=1500, res=300)
model.Cu <- rpart(`Log10 Copper` ~., data = train.data, control=rpart.control(cp=.0001))
par(xpd = NA) # otherwise on some devices the text is clipped
...
#  min.index <- which.min(model$cptable[, "xerror"])
#  return(model$cptable[min.index, "cp"])
#}

#The above didn't work. Instead, we used the cptable to identify the cp value that corresponds to the minimum cross-validation error (xerror). Then, I used this cp value directly in the prune function for all models.

png("CART_Cu_plot_pruned.png", width=2500, height=1500, res=300)
model.Cu.pruned <- prune(model.Cu, cp = 0.07)
par(xpd = NA) # otherwise on some devices the text is clipped
...
#Testing how well tree predicts the test data
pred.Cu_pruned <- predict(model.Cu.pruned, test.data, type="vector")

#Mean Absolute Error
mean(abs(pred.Cu_pruned - test.data$`Log10 Copper`))

#Root Mean Squared Error
sqrt(mean((pred.Cu_pruned - test.data$`Log10 Copper`)^2))

#R-squared 
cor(pred.Cu_pruned, test.data$`Log10 Copper`)^2 

#What about unpruned results?
#Testing how well tree predicts the test data
pred.Cu_full <- predict(model.Cu, test.data, type="vector")

#Mean Absolute Error
mean(abs(pred.Cu_full - test.data$`Log10 Copper`))

#Root Mean Squared Error
sqrt(mean((pred.Cu_full - test.data$`Log10 Copper`)^2))

#R-squared 
cor(pred.Cu_full, test.data$`Log10 Copper`)^2 
...
```{r mercury}
# Split the data into training and test set
#Removing other metals from the Hg analysis. 
Metals_CART_Hg <- Metals_CART %>% 
  dplyr::select(-`Log10 Cadmium`, -`Log10 Manganese`, -`Log10 Copper`, -`Log10 Lead`, -`Log10 Uranium`)
sample(1:100000000,1)
set.seed(8613657)
training.samples <- createDataPartition(Metals_CART_Hg$`Log10 Mercury`, times = 1, p = 0.8, list = F)
train.data  <- Metals_CART_Hg[training.samples, ]
test.data <- Metals_CART_Hg[-training.samples, ]

train.data <- as.data.frame(train.data)

png("CART_Hg_plot.png", width=2500, height=1500, res=300)
model.Hg <- rpart(`Log10 Mercury` ~., data = train.data, control=rpart.control(cp=.0001))
par(xpd = NA) # otherwise on some devices the text is clipped
...
png("CART_Hg_plot_pruned.png", width=2500, height=1500, res=300)
...
summary(model.Hg.pruned)

#Testing how well tree predicts the test data
pred.Hg_pruned <- predict(model.Hg.pruned, test.data, type="vector")

#Mean Absolute Error
mean(abs(pred.Hg_pruned - test.data$`Log10 Mercury`))

#Root Mean Squared Error
sqrt(mean((pred.Hg_pruned - test.data$`Log10 Mercury`)^2))

#R-squared 
cor(pred.Hg_pruned, test.data$`Log10 Mercury`)^2 

#What about unpruned results?
#Testing how well tree predicts the test data
pred.Hg_full <- predict(model.Hg, test.data, type="vector")

#Mean Absolute Error
mean(abs(pred.Hg_full - test.data$`Log10 Mercury`))

#Root Mean Squared Error
sqrt(mean((pred.Hg_full - test.data$`Log10 Mercury`)^2))

#R-squared 
cor(pred.Hg_full, test.data$`Log10 Mercury`)^2 
```
...
Metals_CART_Mn <- Metals_CART %>% 
  dplyr::select(-`Log10 Copper`, -`Log10 Cadmium`, -`Log10 Mercury`, -`Log10 Lead`, -`Log10 Uranium`)
sample(1:100000000,1)
set.seed(96015568)
training.samples <- createDataPartition(Metals_CART_Mn$`Log10 Manganese`, times = 1, p = 0.8, list = F)
train.data  <- Metals_CART_Mn[training.samples, ]
test.data <- Metals_CART_Mn[-training.samples, ]

train.data <- as.data.frame(train.data)

png("CART_Mn_plot.png", width=2500, height=1500, res=300)
model.Mn <- rpart(`Log10 Manganese` ~., data = train.data, control=rpart.control(cp=.0001))
par(xpd = NA) # otherwise on some devices the text is clipped
...
png("CART_Mn_plot_pruned.png", width=2500, height=1500, res=300)
model.Mn.pruned <- prune(model.Mn, cp = 0.02)
par(xpd = NA) # otherwise on some devices the text is clipped
...
#Testing how well tree predicts the test data
pred.Mn_pruned <- predict(model.Mn.pruned, test.data, type="vector")

#Mean Absolute Error
mean(abs(pred.Mn_pruned - test.data$`Log10 Manganese`))

#Root Mean Squared Error
sqrt(mean((pred.Mn_pruned - test.data$`Log10 Manganese`)^2))

#R-squared 
cor(pred.Mn_pruned, test.data$`Log10 Manganese`)^2 

#What about unpruned results?
#Testing how well tree predicts the test data
pred.Mn_full <- predict(model.Mn, test.data, type="vector")

#Mean Absolute Error
mean(abs(pred.Mn_full - test.data$`Log10 Manganese`))

#Root Mean Squared Error
sqrt(mean((pred.Mn_full - test.data$`Log10 Manganese`)^2))

#R-squared 
cor(pred.Mn_full, test.data$`Log10 Manganese`)^2 
```

###Lead
```{r lead}
#Split the data into training and test set
#Removing other metals from the Pb analysis.
Metals_CART_Pb <- Metals_CART %>% 
  dplyr::select(-`Log10 Copper`, -`Log10 Cadmium`, -`Log10 Mercury`, -`Log10 Manganese`, -`Log10 Uranium`)
sample(1:100000000,1)
set.seed(98102741)
training.samples <- createDataPartition(Metals_CART_Pb$`Log10 Lead`, times = 1, p = 0.8, list = F)
train.data  <- Metals_CART_Pb[training.samples, ]
test.data <- Metals_CART_Pb[-training.samples, ]

train.data <- as.data.frame(train.data)

png("CART_Pb_plot.png", width=2500, height=1500, res=300)
labels_TF <- c("true", "false")
model.Pb <- rpart(`Log10 Lead` ~., data = train.data, control=rpart.control(cp=.0001))
par(xpd = NA) # otherwise on some devices the text is clipped
...
png("CART_Pb_plot_pruned.png", width=5000, height=1500, res=300)
model.Pb.pruned <- prune(model.Pb, cp = 0.14)
par(xpd = NA) # otherwise on some devices the text is clipped
prp(model.Pb.pruned, fallen.leaves=FALSE, tweak=1.1, varlen=35, faclen=0, extra=1, uniform=TRUE, yes.text="true", no.text="false")
dev.off()

summary(model.Pb.pruned)

#Testing how well tree predicts the test data
pred.Pb_pruned <- predict(model.Pb.pruned, test.data, type="vector")

#Mean Absolute Error
mean(abs(pred.Pb_pruned - test.data$`Log10 Lead`))

#Root Mean Squared Error
sqrt(mean((pred.Pb_pruned - test.data$`Log10 Lead`)^2))

#R-squared 
cor(pred.Pb_pruned, test.data$`Log10 Lead`)^2 

#What about unpruned results?
#Testing how well tree predicts the test data
pred.Pb_full <- predict(model.Pb, test.data, type="vector")

#Mean Absolute Error
mean(abs(pred.Pb_full - test.data$`Log10 Lead`))

#Root Mean Squared Error
sqrt(mean((pred.Pb_full - test.data$`Log10 Lead`)^2))

#R-squared 
cor(pred.Pb_full, test.data$`Log10 Lead`)^2 
```

###Uranium
```{r uranium}
#Split the data into training and test set
#Removing other metals from the U analysis.
Metals_CART_U <- Metals_CART %>% 
  dplyr::select(-`Log10 Copper`, -`Log10 Cadmium`, -`Log10 Mercury`, -`Log10 Manganese`, -`Log10 Lead`)
sample(1:100000000,1)
set.seed(2603109)
training.samples <- createDataPartition(Metals_CART_U$`Log10 Uranium`, times = 1, p = 0.8, list = F)
train.data  <- Metals_CART_U[training.samples, ]
test.data <- Metals_CART_U[-training.samples, ]

train.data <- as.data.frame(train.data)

png("CART_U_plot.png", width=2500, height=1500, res=300)
model.U <- rpart(`Log10 Uranium` ~., data = train.data, control=rpart.control(cp=.0001))
par(xpd = NA) # otherwise on some devices the text is clipped
...
png("CART_U_plot_pruned.png", width=5000, height=1500, res=300)
model.U.pruned <- prune(model.U, cp = 0.11)
par(xpd = NA) # otherwise on some devices the text is clipped
prp(model.U.pruned, fallen.leaves=FALSE, tweak=1.1, varlen=35, faclen=0, extra=1, uniform=TRUE, yes.text="true", no.text="false")
dev.off()

summary(model.U.pruned)

#Testing how well tree predicts the test data
pred.U_pruned <- predict(model.U.pruned, test.data, type="vector")

#Mean Absolute Error
mean(abs(pred.U_pruned - test.data$`Log10 Uranium`))

#Root Mean Squared Error
sqrt(mean((pred.U_pruned - test.data$`Log10 Uranium`)^2))

#R-squared 
cor(pred.U_pruned, test.data$`Log10 Uranium`)^2 

#What about unpruned results?
#Testing how well tree predicts the test data
pred.U_full <- predict(model.U, test.data, type="vector")

#Mean Absolute Error
mean(abs(pred.U_full - test.data$`Log10 Uranium`))

#Root Mean Squared Error
sqrt(mean((pred.U_full - test.data$`Log10 Uranium`)^2))

#R-squared 
cor(pred.U_full, test.data$`Log10 Uranium`)^2 
```
...
#Recruitment Site:
#Metals_CART2$`Recruitment Site` <- ifelse(Metals_CART2$`Recruitment Site`==1, "CSF",
#                                          ifelse(Metals_CART2$`Recruitment Site`==2, "RCBH",
#                                                 ifelse(Metals_CART2$`Recruitment Site`==3, "YRMC", NA)))

#Metals_CART$`Recruitment Site` <- as.factor(Metals_CART$`Recruitment Site`)

#Water: 
#Metals_CART2$`Water Source` <- ifelse(Metals_CART2$`Water Source`==1, "Tap",
#                                      ifelse(Metals_CART2$`Water Source`==2, "Well",
#                                             ifelse(Metals_CART2$`Water Source`==3, "Bottled",
#                                                    ifelse(Metals_CART2$`Water Source`==4, "Other",
#                                                           ifelse(Metals_CART2$`Water Source`==5, "Mixed",NA)))))

#Metals_CART$`Water Source` <- as.factor(Metals_CART$`Water Source`)

#Residence Type:
#Metals_CART2$`Residence Type` <- ifelse(Metals_CART2$`Residence Type`==1, "House",
#                                        ifelse(Metals_CART2$`Residence Type`==2, "Apt",
#                                               ifelse(Metals_CART2$`Residence Type`==3, "Manufactured",
#                                                      ifelse(Metals_CART2$`Residence Type`==4, "Hotel/Motel",
#                                                             ifelse(Metals_CART2$`Residence Type`==5, "Homeless",
#                                                                    ifelse(Metals_CART2$`Residence Type`==6, "Group", 
#                                                                           ifelse(Metals_CART2$`Residence Type`==7, "Other", 
#                                                                                  ifelse(Metals_CART2$`Residence Type`==8, "Refused",NA))))))))

#Metals_CART$`Residence Type` <- as.factor(Metals_CART$`Residence Type`)

#Education:
#Metals_CART2$Education <- ifelse(Metals_CART2$Education==1, "None", 
#                                 ifelse(Metals_CART2$Education==2, "Elementary",
#                                        ifelse(Metals_CART2$Education==3, "Middle",
#                                               ifelse(Metals_CART2$Education==4, "High",
#                                                      ifelse(Metals_CART2$Education==5, "College",
#                                                             ifelse(Metals_CART2$Education==6, "Advanced", 
#                                                                    ifelse(Metals_CART2$Education==7, "Other", NA)))))))

#Metals_CART$Education <- as.factor(Metals_CART$Education)

#Health Compared to last year:
#Metals_CART2$`Health Compared to Last Year` <- ifelse(Metals_CART2$`Health Compared to Last Year`==1, "Better", 
#                                                      ifelse(Metals_CART2$`Health Compared to Last Year`==2, "Worse",
#                                                             ifelse(Metals_CART2$`Health Compared to Last Year`==3, "Same",
#                                                                    ifelse(Metals_CART2$`Health Compared to Last Year`==4, "Don't know",
#                                                                           ifelse(Metals_CART2$`Health Compared to Last Year`==5, "Refused", NA)))))

#Metals_CART$`Health Compared to Last Year` <- as.factor(Metals_CART$`Health Compared to Last Year`)

#Ethnicity:
#Metals_CART2$Ethnicity <- ifelse(Metals_CART2$Ethnicity==1, "Hispanic",
#                                 ifelse(Metals_CART2$Ethnicity==2, "Non-Hispanic", NA))

#Metals_CART$Ethnicity <- as.factor(Metals_CART$Ethnicity)

#Race:
#Metals_CART2$Race <- ifelse(Metals_CART2$Race==1,"American Indian or Alaska Native",
#                            ifelse(Metals_CART2$Race==2, "Asian",
#                                   ifelse(Metals_CART2$Race==3, "Black or AA",
#                                          ifelse(Metals_CART2$Race==4, "Native Hawaiian or Pacific Islander",
#                                                 ifelse(Metals_CART2$Race==5, "White",
#                                                        ifelse(Metals_CART2$Race==6, "Other",
#                                                               ifelse(Metals_CART2$Race==7, "Don't know",
#                                                                      ifelse(Metals_CART2$Race==8, "Refused",
#                                                                             ifelse(Metals_CART2$Race==9, "Multiple", NA)))))))))
...
#model.PC1 <- rpart(PC1 ~., data = train.data, control=rpart.control(cp=.0001))
#par(xpd = NA) # otherwise on some devices the text is clipped
#rpart.plot(model.PC1, fallen.leaves=FALSE, tweak=1.1, varlen=35, faclen=0, extra=1, uniform=TRUE)
#dev.off()

#png("CART_PC1_plot_pruned.png", width=1200, height=800, res=300)
#model.PC1.pruned <- prune(model.PC1, cp = 0.02)
#par(xpd = NA) # otherwise on some devices the text is clipped
#prp(model.PC1.pruned, fallen.leaves=FALSE, tweak=1.1, varlen=35, faclen=0, extra=1, uniform=TRUE)
#dev.off()
...
```{r PC2_CART}
#Metals_CART_PC2 <- Metals_CART2 %>% 
#  dplyr::select(-PC1)
#sample(1:100000000,1)
#set.seed(1490797)
#training.samples <- createDataPartition(Metals_CART_PC2$PC2, times = 1, p = 0.8, list = F)
#train.data  <- Metals_CART_PC2[training.samples, ]
#test.data <- Metals_CART_PC2[-training.samples, ]

#train.data <- as.data.frame(train.data)

#png("CART_PC2_plot.png", width=1200, height=800, res=300)
#model.PC2 <- rpart(PC2 ~., data = train.data, control=rpart.control(cp=.0001))
#par(xpd = NA) # otherwise on some devices the text is clipped
#rpart.plot(model.PC2, fallen.leaves=FALSE, tweak=1.1, varlen=35, faclen=0, extra=1, uniform=TRUE)
#dev.off()

#png("CART_PC2_plot_pruned.png", width=1200, height=800, res=300)
#model.PC2.pruned <- prune(model.PC2, cp = 0.02)
#par(xpd = NA) # otherwise on some devices the text is clipped
#rpart.plot(model.PC2.pruned, fallen.leaves=FALSE, tweak=1.1, varlen=35, faclen=0, extra=1, uniform=TRUE)
#dev.off()

#Testing how well tree predicts the test data
#pred.PC2_pruned <- predict(model.PC2.pruned, test.data, type="vector")

#Mean Absolute Error
#mean(abs(pred.PC2_pruned - test.data$PC2))

#Root Mean Squared Error
#sqrt(mean((pred.PC2_pruned - test.data$PC2)^2))

#R-squared 
#cor(pred.PC2_pruned, test.data$PC2)^2 #25.7%

#What about unpruned results?
#Testing how well tree predicts the test data
#pred.PC2_full <- predict(model.PC2, test.data, type="vector")

#Mean Absolute Error
#mean(abs(pred.PC2_full - test.data$PC2))

#Root Mean Squared Error
#sqrt(mean((pred.PC2_full - test.data$PC2)^2))

#R-squared 
#cor(pred.PC2_full, test.data$PC2)^2 #13.0%
```
